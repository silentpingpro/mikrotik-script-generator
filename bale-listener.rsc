:local token "34987319:WwjB-dM5bEPXYDs5ffVG8jKN7GRk-jduVuI"
:local chatid "1644670601"
:local offset 1
:local api "https://tapi.bale.ai/bot"

:local ifname [/interface bridge find where name~"bale-offset-"]
:if ([:len $ifname] > 0) do={
    :local fullname [/interface bridge get $ifname name]
    :set offset [:tonum [:pick $fullname 12 [:len $fullname]]]
}

:local fetchErr 0
:do { /tool fetch url=($api . $token . "/getUpdates?offset=" . $offset . "&timeout=0") check-certificate=no keep-result=yes dst-path=bale-updates.txt } on-error={ :set fetchErr 1 }
:if ($fetchErr != 0) do={ :return }
:local data ""
:do { :set data [/file get bale-updates.txt contents] } on-error={ :set data "" }
...
:if (($msgtext = "/start") || ($msgtext = "/menu")) do={
    /tool fetch url=($api . $token . "/sendMessage?chat_id=" . $chatid . "&text=%F0%9F%93%A1+%D9%BE%D9%86%D9%84+%DA%A9%D9%86%D8%AA%D8%B1%D9%84+MikroTik&reply_markup=" . $mainKbd) check-certificate=no keep-result=no
    :set replied true
}
...
:if (($msgtext = "/status") && ($replied=false)) do={
    ...
    /tool fetch url=($api . $token . "/sendMessage?chat_id=" . $chatid . "&text=" . $msg . "&reply_markup=" . $mainKbd) check-certificate=no keep-result=no
    :set replied true
}
...
:if ($isCallback=true) do={
    ...
    :if ($cdata = "server_menu") do={
        /tool fetch url=($api . $token . "/sendMessage?chat_id=" . $chatid . "&text=%F0%9F%96%A5%EF%B8%8F+%D8%B3%D8%B1%D9%88%D8%B1%D9%87%D8%A7%3A&reply_markup=" . $srvMenuKbd) check-certificate=no keep-result=no
    }
    ...
    :foreach idx in={1;2;3;4;5;6;7} do={
        ...
        :if ($cdata = ("srv_" . $idx)) do={
            ...
            /tool fetch url=($api . $token . "/sendMessage?chat_id=" . $chatid . "&text=" . $msg . "&reply_markup=" . $ctrlK) check-certificate=no keep-result=no
        }
...
:if ([:find $data "update_id"] > 0) do={
    ...
    :if ([:len $ifname] > 0) do={ /interface bridge set $ifname name=("bale-offset-" . $newoffset) }
    :if ([:len $ifname] = 0) do={ /interface bridge add name=("bale-offset-" . $newoffset) }
}